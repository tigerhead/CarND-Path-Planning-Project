      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            if(prev_points > 0){//if there is previous path point, use last prevoise path s as car_s
                car_s = end_path_s;
            }
            ...
            for(int i=0; i<sensor_fusion.size(); i++)
            {
                //determine if car is directly ahead
                double check_car_d = sensor_fusion[i][6];
                if (check_car_d < (2 + 4 * current_lane + 2 ) && check_car_d > (2 + 4 * current_lane - 2))// Car is in the same lane
                {
                    double vx = sensor_fusion[i][3];
                    double vy = sensor_fusion[i][4];
                    double check_car_speed = sqrt(vx*vx + vy*vy);
            ...
            if(full_break){//emergency happened, car ahead is too close
                ...
                if(too_close){
                    ...
                    if(target_lane >= 0 && target_lane <=2){
                        ...
                        target_lane = current_lane; //Left lance change is not possible

                    }


                    if(target_lane == current_lane){ //No left lane change, try right lane change
                        ...
                        if(target_lane >= 0 && target_lane <=2){
                            ...
                            target_lane = current_lane; //right lance change is not possible
           ...
           // cout << check_distance << "\n";
            ...
            double target_x = distance_interval;
